...
<style>
    .time-slot-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 12px; margin-top: 20px; }
    .time-slot { padding: 15px; border: 2px solid #e9ecef; border-radius: 8px; text-align: center; cursor: pointer; transition: 0.2s; font-weight: 600; background: #fff; }
    .time-slot.available:hover { border-color: #0d6efd; color: #0d6efd; background: #f8f9ff; }
    .time-slot.selected { background: #0d6efd !important; color: #fff !important; border-color: #0d6efd !important; }
    .time-slot.booked { background: #f8d7da; color: #721c24; border-color: #f5c6cb; cursor: not-allowed; opacity: 0.7; }
    
    /* Calendar Highlighting */
    .flatpickr-day.enabled-day { background: #e8f5e9 !important; border-color: #2e7d32 !important; color: #2e7d32 !important; font-weight: bold; }
    .flatpickr-day.enabled-day:after { content: ""; position: absolute; bottom: 3px; left: 50%; transform: translateX(-50%); width: 4px; height: 4px; border-radius: 50%; background: #2e7d32; }
    
    .booking-legend span { margin-right: 15px; font-size: 0.9rem; }
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <ul class="nav nav-pills" id="booking-tabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button 
                class="nav-link {{ request()->get('mybooking') !== 'active' ? 'active' : '' }}" 
                id="tab-book" 
                data-bs-toggle="pill" 
                data-bs-target="#content-book" 
                type="button" 
                aria-selected="{{ request()->get('mybooking') !== 'active' ? 'true' : 'false' }}" 
                role="tab">
        ...
        <li class="nav-item" role="presentation">
            <button 
                ...
                id="tab-history" 
                data-bs-toggle="pill" 
                data-bs-target="#content-history" 
                type="button" 
                aria-selected="{{ request()->get('mybooking') === 'active' ? 'true' : 'false' }}" 
                tabindex="-1" 
                role="tab">
...
<div class="tab-content">
    ...
    <div class="tab-pane fade {{ request()->get('mybooking') !== 'active' ? 'show active' : '' }}" 
        id="content-book" 
        role="tabpanel" 
        ...
        <div class="row">
                ...
                <div class="card shadow-sm border-0 mb-4">
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="bookingDate" placeholder="Select a facility first..." readonly>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-8">
                <div class="card shadow-sm border-0">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="mb-0">2. Select Time for <span id="selectedDateDisplay" class="text-primary">Today</span></h5>
                        ...
                        <div id="confirmation-section" class="mt-5 pt-4 border-top text-center" style="display: none;">
                            <p class="lead">Confirm booking for <strong id="confirm-details" class="text-primary"></strong>?</p>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    facilitySelect.addEventListener('change', function() {
        ...
        slotContainer.innerHTML = '<div class="text-muted py-5 text-center w-100"><p>Loading schedule...</p></div>';
        ...
        $.get("{{ url('member/get-facility-days') }}/" + facilityId, function(days) {
            ...
            if (enabledDayIndices.includes(todayIdx)) {
                ...
                slotContainer.innerHTML = '<div class="alert alert-info w-100 text-center">Facility is closed today. Please pick a highlighted date.</div>';
            }
        });
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (!response.success || response.slots.length === 0) {
                    slotContainer.innerHTML = `<div class="alert alert-warning w-100">${response.message || 'No slots available for this day.'}</div>`;
                    return;
                }
            ...
            error: function() {
                slotLoader.style.display = 'none';
                slotContainer.innerHTML = '<div class="alert alert-danger w-100">Error loading slots.</div>';
            }
        });
    }
